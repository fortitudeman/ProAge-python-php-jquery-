...
class Exec extends CI_Controller {
	...
	public function repairNegociosPai(){
		$query = $this->db->query('select * from payments where (select count(*) from policy_negocio_pai where policy_number = payments.policy_number) = 0 and year_prime = 1 and valid_for_report = 1 ORDER BY policy_number asc, payment_date asc');
		...
		foreach ($policies as $policy_id => $policy) {
			...
			foreach ($policy as $payment) {
				...
				if($flag && $total > 12000){
					...
					$flag = false;
					echo $policy_id." $payment_date<br>";
					$count++;
				}
			 }
		}
