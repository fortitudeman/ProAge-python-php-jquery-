        ...
        private function _getNegocioPai( $count_requested = TRUE, $agent_id = null, $filter = array())
	{
		...
		if (!$count_requested)
		{
			...
			$join_plus = ' LEFT OUTER JOIN `policies` ON `policies`.`uid`= `payments`.`policy_number` ';
                ...
                $sql_str = "SELECT `policy_negocio_pai`.*,`policy_negocio_pai`.id negocio_pai_id, ".$select_plus." `payments`.* , `users`.`name` AS `first_name`, `users`.`lastnames` AS `last_name`, `users`.`company_name` AS `company_name` FROM `payments` 
